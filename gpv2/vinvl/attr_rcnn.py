...
class AttrRCNN(GeneralizedRCNN):
  ...
  def forward(self, images, targets=None, return_backbone_features=False):
    """
    Arguments:
        images (list[Tensor] or ImageList): images to be processed
        targets (list[BoxList]): ground-truth boxes present in the image (optional)
        boxes Tensor: optional boxes to extract features for
    ...
    # else trust the client to get this right
    features = self.backbone(images.tensors)

    if targets and self.device is not None:
    ...
    if self.training and not self.always_return_predictions:
      losses = {}
